...
exports.run = async(client, message, args) => {
  ...
  if (!message.member.hasPermission("MANAGE_ROLES")){
    ...
    .setColor('RED')
    .setDescription(`:x:  Bu komutu kullanabilmek için \`Rolleri Yönet\` Adlı Yetki Gerekiyor !`)
    ...
    if (!args[0]) {
        ...
        var embed = new Discord.RichEmbed()
            ...
            .addField('Diğer Bilgiler', `**Entegrasyonmu: ** ${rol.managed ? ':white_check_mark:' : ':x: '}\n**Etiketlenebilirmi: ** ${rol.mentionable ? ':white_check_mark:' : ':x: '}\n**Pozisyonu: ** ${rol.position}\n**Ayrı Gösteriliyormu:** ${rol.hoisted ? ':white_check_mark:' : ':x:'} `)
            .setFooter(`${client.user.tag} | ${prefix}yardım`)
            .setTimestamp()
             message.channel.send(embed);
      }
